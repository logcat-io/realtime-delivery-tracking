...
추적에서 사용자가 알고 싶은 건 지나온 모든 상태가 아니라 **지금 어디까지 왔는지**다. 지나온 이력이 필요하면 `delivery_status_history` 를 조회하면 된다. 이력은 남아 있고, 추적은 다른 일이다 — 스트림에 재생을 얹으면 같은 데이터를 두 경로가 서로 다르게 주게 되고, 어긋났을 때 어느 쪽이 맞는지 판단할 근거가 없어진다.
